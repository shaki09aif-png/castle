...
function createFlag(scene, pos, uTime) {
  const W = 3.8, H = 2.4;
  const geo = new THREE.PlaneGeometry(W, H, 28, 14);
  geo.translate(W / 2, -H / 2, 0);
  const mat = new THREE.MeshStandardMaterial({ map: bannerTexture(), side: THREE.DoubleSide, roughness: 0.85 });
  ...
  scene.add(group);
  return (t) => {
    group.rotation.y = 0.6 + Math.sin(t * 0.37) * 0.25; // по ветру, как флюгеры
  };
}
